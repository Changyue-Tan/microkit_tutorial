...
void notified(microkit_channel channel) {
	if (channel == UART_CHANNEL_ID) {
		// something was inputed via keyboard
		char input = uart_get_char();

		input_buffer[0] = input;  // write input char to input buffer
		microkit_notify(
		    CHANNEL_TO_CLIENT);  // tell the clinet to display the char

		uart_handle_irq();
		microkit_irq_ack(channel);
	} else if (channel ==
	           CHANNEL_TO_CLIENT) {  // clients wants to display something
