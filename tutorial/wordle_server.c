...
microkit_msginfo protected(microkit_channel channel, microkit_msginfo msginfo) {
    ...
    microkit_msginfo message_structure_to_be_returned = microkit_msginfo_new(0, 0);
    ...
    switch (channel) {
        case CHANNEL_TO_CLIENT: 
            ...
            message_structure_to_be_returned = microkit_msginfo_new(0, WORDLE_WORD_SIZE);
            for (int i = 0; i < WORDLE_WORD_SIZE; i++) {
                microkit_mr_set(i, entered_word_states[i]);
            }
            break;
        case CHANNEL_TO_VMM:
            int update_word_from_remote_server[WORDLE_WORD_SIZE];
